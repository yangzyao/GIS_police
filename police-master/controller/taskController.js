                    ...
                    conn.query(sql,[taskId],function(err,data){
                        if(data[0].total==0){
                            ...
                            /*
                                ...
                                4:审核不通过
                            */
                            conn.query("update task set taskStatus=2,cameraLon=?,cameraLa=? where Id=?",[cameraLon,cameraLa，taskId],
                                function(err,data){
                                    if(err){
                                        console.log(err);
                                        res.json({ "code": 300, "data": { "status": "fail", "error": "修改任务状态失败" } });
                                    } else {
                                        var sqls="select * from taskfeedback where tsdkId=?";
                                        conn.query(sqls,[taskId],function (err,datq) {
                                            if(err){
                                                res.json({ "code": 300, "data": { "status": "fail", "error": "数据库查询失败" } });
                                            }else{
                                                if(datq.length>0){
                                                    var sql ="updata taskfeedback set content=?,cameraLon=?,cameraLa=? where taskId=?";
                                                    var dataArr=[content,cameraLon,cameraLa,taskId];
                                                    conn.query(sql,dataArr,function (err,datas) {
                                                        if(err){
                                                            res.json({ "code": 300, "data": { "status": "fail", "error": "数据库更新失败" } });
                                                        }else{
                                                            Log.insertLog(mobile,"更新任务",sql);
                                                            res.json({ "code": 200, "data": { "status": "success", "error": "success" } });
                                                        }
